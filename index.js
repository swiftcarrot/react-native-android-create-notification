...
const { NotificationModule } = require('react-native').NativeModules;

// Warp the native module so we can do some pre/post processing to have a cleaner API.
const Notification = {
  create(attributes = {}) {
    console.log('notification.create', attributes);
  ...
  addListener: function(type, listener) {
    switch (type) {
      ...
      case 'click':
        ...
        // NotificationModule.getInitialSysNotification(function(
        //   initialSysNotificationId,
  ...
  // getInitialSysNotification: NotificationModule.getInitialSysNotification,
  // removeInitialSysNotification: NotificationModule.removeInitialSysNotification,
...
function encodeNativeNotification(attributes) {
  ...
  if (attributes.sendAt !== undefined) {
    ...
    if (attributes.repeatCount) {
      if (typeof attributes.repeatEvery === 'number') {
        attributes.endAt = parseInt(
          attributes.sendAt +
            attributes.repeatEvery * attributes.repeatCount +
            attributes.repeatEvery / 2
        );
      } else if (typeof attributes.repeatEvery === 'string') {
        switch (attributes.repeatEvery) {
          case 'minute':
            attributes.endAt =
              attributes.sendAt + 60000 * attributes.repeatCount + 1000 * 30;
            break;

          case 'hour':
            attributes.endAt =
              attributes.sendAt +
              60000 * 60 * attributes.repeatCount +
              60000 * 30;
            break;
          ...
          case 'halfDay':
            attributes.endAt =
              attributes.sendAt +
              60000 * 60 * 12 * attributes.repeatCount +
              60000 * 60 * 6;
            break;
          ...
          case 'day':
            attributes.endAt =
              attributes.sendAt +
              60000 * 60 * 24 * attributes.repeatCount +
              60000 * 60 * 12;
            break;
          ...
          case 'week':
            attributes.endAt =
              attributes.sendAt +
              60000 * 60 * 24 * 7 * attributes.repeatCount +
              60000 * 60 * 24 * 3;
            break;
          ...
          case 'month':
            attributes.endAt =
              attributes.sendAt +
              60000 * 60 * 24 * 30 * attributes.repeatCount +
              60000 * 60 * 24 * 15;
            break;
          ...
          case 'year':
            attributes.endAt =
              attributes.sendAt +
              60000 * 60 * 24 * 365 * attributes.repeatCount +
              60000 * 60 * 24 * 100;
            break;
        }
      }
