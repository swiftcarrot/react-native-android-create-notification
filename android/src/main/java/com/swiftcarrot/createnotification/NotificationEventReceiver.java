...
public class NotificationEventReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (!applicationIsRunning(context)) {
            ...
            // todo: FLAG_ACTIVITY_CLEAR_TASK
            launchIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);

            context.startActivity(launchIntent);
            Log.i("ReactSystemNotification", "NotificationEventReceiver: Launching: " + packageName);
        } else {
            // If the application is already running in foreground, send a brodcast too
